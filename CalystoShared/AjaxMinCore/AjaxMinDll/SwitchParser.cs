...
namespace Microsoft.Ajax.Utilities
{
    ...
    public class SwitchParser
    {
        ...
        public SwitchParser(CodeSettings scriptSettings)
        {
        ...
        {
            // clone the settings
            var newParser = new SwitchParser(this.JSSettings.Clone());
                        ...
                        switch (switchPart)
                        {
                            ...
                            case "LINES":
                                if (string.IsNullOrEmpty(paramPartUpper))
                                {
                                    ...
                                    JSSettings.LineBreakThreshold = int.MaxValue - 1000;
                                    
                                    // single-line mode
                                    JSSettings.OutputMode = OutputMode.SingleLine;

                                    // and four spaces per indent level
                                    JSSettings.IndentSize = 4;
                                }
                                else
                                {
                                    ...
                                    if (lineParts.Length <= 3)
                                    {
                                        ...
                                        if (!string.IsNullOrEmpty(lineParts[0]))
                                        {
                                            ...
                                            {
                                                JSSettings.LineBreakThreshold = lineThreshold;
                                            }
                                            ...
                                            {
                                                // multiple-line mode
                                                JSSettings.OutputMode = OutputMode.MultipleLines;
                                            ...
                                            if (lineParts.Length > breakIndex)
                                            {
                                                ...
                                                if (!string.IsNullOrEmpty(lineParts[breakIndex]))
                                                {
                                                    ...
                                                    if (lineParts[breakIndex].TryParseIntInvariant(NumberStyles.None, out indentSize))
                                                    {
                                                        ...
                                                        JSSettings.IndentSize = indentSize;
                                                    }
                                                    else
                                                    {
                                                        OnInvalidSwitch(switchPart, lineParts[breakIndex]);
                            ...
                            case "TERM":
                                ...
                                if (BooleanSwitch(paramPartUpper, true, out parameterFlag))
                                {
                                    JSSettings.TermSemicolons = parameterFlag;
                                }
                                else
                                {
                                    OnInvalidSwitch(switchPart, paramPart);
                            ...
                            case "Z":
                                ...
                                JSSettings.TermSemicolons = true;
                                break;
