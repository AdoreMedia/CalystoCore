...
namespace Microsoft.Ajax.Utilities
{
    ...
    internal class AnalyzeNodeVisitor : TreeVisitor
    {
            ...
            if (node != null)
            {
                ...
                else
                {
                    ...
                    else if (node.IsAssign)
                    {
                        ...
                        && (node.OperatorToken == JSToken.LogicalOr || node.OperatorToken  == JSToken.LogicalOrElvis || node.OperatorToken == JSToken.LogicalAnd))
                    {
        ...
        private void CombineWithPreviousExpression(Block node, int ndx)
        {
            ...
            {
                // gurufix: do not combine return, it makes mess
                                ...
                                if (varField.RefCount == 0)
                                {
                                    ...
                                    if (nameDecl != null)
                                    {
                                        ...
                                        if (nameDecl.Initializer == null || nameDecl.Initializer.IsConstant)
                                        {
                                            ...
                                            // maybe: would be nice to properly remove unused declarations from binding parameters
                                ...
                                if (forNode.Initializer != null)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        if (binaryOp != null && AreAssignmentsInVar(binaryOp, previousVar))
                                        {
                                            ...
                                            // maybe: we *could* also do it is the expr1 assignments are to lookups that are
                                ...
                                && ifNode.TrueBlock.Count == 1)
                            {
                                    ...
                                    && (string.IsNullOrEmpty(continueNode.Label) || (LabelMatchesParent(continueNode.Label, node.Parent))))
                                {
                                    ...
                                    if (ndx < node.Count - 1)
                                    {
                                        ...
                                        // maybe: if we removed a labeled continue, do we need to fix up some label references?
        ...
        public override void Visit(ComprehensionNode node)
        {
            if (node != null)
            {
                ...
                // maybe: verify that there are no naming collisions in the scope
            }
        }
