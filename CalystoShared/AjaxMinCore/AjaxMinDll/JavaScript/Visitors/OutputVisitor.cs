...
namespace Microsoft.Ajax.Utilities
{
    ...
    public class OutputVisitor : IVisitor
    {
        ...
        private OutputVisitor(TextWriter writer, CodeSettings settings, ScriptVersion scriptVersion)
        {
        ...
        public static void Apply(TextWriter writer, AstNode node, CodeSettings settings, ScriptVersion scriptVersion = ScriptVersion.EcmaScript5)
        {
            if (node != null)
            {
                var outputVisitor = new OutputVisitor(writer, settings, scriptVersion);
        ...
        public static string Apply(AstNode node, CodeSettings settings, ScriptVersion scriptVersion = ScriptVersion.EcmaScript5)
        {
            if (node != null)
            {
                using (var writer = new StringWriter(CultureInfo.InvariantCulture))
                {
                    OutputVisitor.Apply(writer, node, settings, scriptVersion);
                                ...
                                && ourPrecedence != OperatorPrecedence.Comma)
                            {
                                if (node.OperatorToken == rightHandBinary.OperatorToken)
                                {
                                    ...
                                    switch (node.OperatorToken)
                                    {
                                        ...
                                        // maybe: the plus operator: if we can prove that it is a numeric operator
        ...
        public void Visit(IfNode node)
        {
            if (node != null)
            {
                ...
                if (node.TrueBlock != null && ( node.TrueBlock.ForceBraces || m_scriptVersion == ScriptVersion.EcmaScript6))
                {
        ...
        public void Visit(Member node)
        {
            if (node != null)
            {
                ...
                if (node.Name != null) // my?.["prop1"] will create the last not without name
                {
                    OutputPossibleLineBreak('.');
		...
		{
            this.VisitReturnOrYield(node, node.Operand, "return");
		}

        public void Visit(YieldNode node)
		{
            this.VisitReturnOrYield(node, node.Operand, "yield");
        }

        private void VisitReturnOrYield(AstNode node, AstNode operand, string token)
        {
            if (node != null)
            {
                var symbol = StartSymbol(node);
        ...
        public void Visit(AwaitNode node)
        {
            this.VisitReturnOrYield(node, node.Operand, "await");
        }
        ...
        public void Visit(TryNode node)
        {
            if (node != null)
            {
                ...
                if (node.CatchParameter != null || node.CatchBlock != null) // es6 may have catch without paramter (...)
                {
                    hasCatchBlock = true;
        ...
        public static string OperatorString(JSToken token)
        {
            switch (token)
            {
                ...
                case JSToken.LogicalOrElvis: return "??";
        ...
        {
            if (block != null && (block.ForceBraces || m_scriptVersion == ScriptVersion.EcmaScript6))
            {
