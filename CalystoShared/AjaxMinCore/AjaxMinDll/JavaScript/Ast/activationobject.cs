...
namespace Microsoft.Ajax.Utilities
{
    ...
    public abstract class ActivationObject
    {
                ...
                if (varDecl != null)
                {
                        ...
                        && (varDecl.Initializer == null || varDecl.Initializer.IsConstant))
                    {
                            ...
                            && declaration == forInStatement.Variable)
                        {
                            ...
                            // maybe: try to reuse some pre-existing variable, or maybe replace
                    ...
                    foreach (var varField in NameTable.Values)
                    {
                            ...
                            && (varField.FieldType != FieldType.Arguments && varField.FieldType != FieldType.Predefined))
                        {
                            ...
                            if (!string.IsNullOrEmpty(newName))
                            {
                                ...
                                // maybe: should we at least throw a warning?
		...
		{
            this.ValidateGeneratedNames(new HashSet<string>());
		}

        internal void ValidateGeneratedNames(HashSet<string> usedNames)
        {
            ...
            foreach (JSVariableField variableField in NameTable.Values)
            {
                    ...
                    && variableField.CrunchedName == null)
                {
                    ...
                    string name1 = crunchEnum.NextName(usedNames);

                    // and use it to generate a new name
                    variableField.CrunchedName = name1;
            ...
            foreach (ActivationObject scope in ChildScopes)
            {
                if (!scope.Existing)
                {
                    scope.ValidateGeneratedNames(usedNames);
                }
            }
        }
		...
		{
            this.AutoRenameFields(new HashSet<string>());
		}

        internal virtual void AutoRenameFields(HashSet<string> usedNames)
        {
            ...
            foreach (ActivationObject scope in ChildScopes)
            {
                scope.AutoRenameFields(usedNames);
            }
        }
