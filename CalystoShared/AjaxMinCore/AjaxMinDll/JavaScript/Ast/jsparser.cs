...
namespace Microsoft.Ajax.Utilities
{
    ...
    public class JSParser
    {
        ...
        private AstNode ParseStatement(bool fSourceElement, bool skipImportantComment = false)
        {
            ...
            else
            {
                switch (m_currentToken.Token)
                {
                    ...
                    case JSToken.Yield: // <<< gurufix, added yield
                        return ParseYieldStatement();

                    case JSToken.Await: // <<< gurufix, added await
                    ...
                    case JSToken.Function:
                        ...
                        var function = ParseFunction(FunctionType.Declaration, m_currentToken.Clone(), false);
        ...
        private VariableDeclaration ParseVarDecl(JSToken inToken)
        {
            ...
            if (binding != null)
            {
                ...
                if (m_currentToken.IsEither(JSToken.Assign, JSToken.Equal))
                {
					...
					else if (m_currentToken.Is(JSToken.Yield))
					{
						initializer = ParseYieldStatement();
					}
                    else if (m_currentToken.Is(JSToken.Await))
					{
                        initializer = ParseAwaitStatement();
					}
        ...
        // ParseYieldStatement
        private YieldNode ParseYieldStatement()
        {
            ParsedVersion = ScriptVersion.EcmaScript6;

            var yieldNode = new YieldNode(m_currentToken.Clone());
            GetNextToken();
            ...
            if (!m_foundEndOfLine)
            {
                if (m_currentToken.IsNot(JSToken.Semicolon) && m_currentToken.IsNot(JSToken.RightCurly))
                {
                    ...
                    {
                        yieldNode.UpdateWith(yieldNode.Operand.Context);
                    }
                }

                ExpectSemicolon(yieldNode);
            }
            else
            {
                // but we did find a line-break -- semicolon-insertion rules have kicked in
                ReportError(JSError.SemicolonInsertion, yieldNode.Context.FlattenToEnd());
            }
        ...
        // ParseAwaitStatement
        private AwaitNode ParseAwaitStatement()
        {
            ParsedVersion = ScriptVersion.EcmaScript6;

            var awaitNode = new AwaitNode(m_currentToken.Clone());
            GetNextToken();
            ...
            if (!m_foundEndOfLine)
            {
                if (m_currentToken.IsNot(JSToken.Semicolon) && m_currentToken.IsNot(JSToken.RightCurly))
                {
                    awaitNode.Operand = ParseExpression();
                    if (awaitNode.Operand != null)
                    {
                        awaitNode.UpdateWith(awaitNode.Operand.Context);
                    }
                }

                ExpectSemicolon(awaitNode);
            }
            else
            {
                // but we did find a line-break -- semicolon-insertion rules have kicked in
                ReportError(JSError.SemicolonInsertion, awaitNode.Context.FlattenToEnd());
            }
        ...
        private AstNode ParseTryStatement()
        {
            ...
            if (m_currentToken.Is(JSToken.Catch))
            {
                ...
                if (m_currentToken.IsNot(JSToken.LeftParenthesis))
                {
                    // ScriptVersion.EcmaScript6 may have catch without (...)
                    if (this.ParsedVersion != ScriptVersion.EcmaScript6)
                    {
                        ReportError(JSError.NoLeftParenthesis);
                    }
                }
        ...
        private AstNode ParseExport()
        {
            ...
            GetNextToken();
            if (m_currentToken.IsOne(JSToken.Var, JSToken.Const, JSToken.Let, JSToken.Function, JSToken.Async, JSToken.Class))
            {
        ...
        //---------------------------------------------------------------------------------------
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Maintainability", "CA1502:AvoidExcessiveComplexity")]
        private FunctionObject ParseFunction(FunctionType functionType, Context fncCtx, bool isAsyncMethod)
        {
			while (m_currentToken.Is(JSToken.Async))
			{
            ...
            if (m_currentToken.Is(JSToken.Identifier)
                // gurufix: allow "catch" and "finally" as function names (Promise has that function names)
                || m_currentToken.Is(JSToken.Catch)
                || m_currentToken.Is(JSToken.Try)
                || m_currentToken.Is(JSToken.Finally)
                || m_currentToken.Is(JSToken.Delete)
            )
            {
        ...
        private AstNode ParseClassElement()
        {
            ...
            if(m_currentToken.Is(JSToken.Get) && this.PeekToken() == JSToken.Identifier)
			{
                // getter property: get Isdone(){return true;}
                functionType = FunctionType.Getter;
			}
            else if(m_currentToken.Is(JSToken.Set) && this.PeekToken() == JSToken.Identifier)
			{
                // setter property: set Isdone(value){...}
                functionType = FunctionType.Setter;
			}
			else
			{
                // method get(size){...} or set(size){...}
                functionType = FunctionType.Method;
			}

            // right now the ES6 spec just has method declarations.
            var method = ParseFunction(functionType, m_currentToken.FlattenToStart(), false);
        ...
        TryItAgain:
            ...
            switch (opToken)
            {
                case JSToken.RestSpread:
                    ...
                    // maybe: we want to error if we aren't immediately preceeded by a comma operator,
        ...
        TryItAgain:
            ...
            {
                case JSToken.Await:
                    ast = new AwaitNode(m_currentToken.Clone());
                    GetNextToken();
                    ((AwaitNode)ast).Operand = ParseExpression();
                    break;
                ...
                // function expression
                case JSToken.Function:
                    ast = ParseFunction(FunctionType.Expression, m_currentToken.Clone(), isAsyncMethod);
                    break;
            ...
            {
                ParsedVersion = ScriptVersion.EcmaScript6;
                ast = ParseArrowFunction(ast, isAsyncMethod);
            }
        ...
        private TemplateLiteral ParseTemplateLiteral()
        {
            ...
            if (indexBackquote != 0)
            {
                ...
                // maybe: figure out how to get a context on just the literal part!
                ...
                do
                {
                    ...
                    if (m_currentToken.Is(JSToken.RightCurly))
                    {
                        ...
                        if (m_currentToken.Is(JSToken.TemplateLiteral))
                        {
                            ...
                            // gurufix: update context
                            templateLiteral.UpdateWith(m_currentToken.Clone());
        ...
        private FunctionObject ParseArrowFunction(AstNode parameters, bool isAsyncMethod)
        {
        ...
        private ObjectLiteral ParseObjectLiteral(bool isBindingPattern)
        {
            ...
            do
            {
                ...
                if (m_currentToken.IsNot(JSToken.RightCurly))
                {
                    ...
                    if (spreadContext != null)
                    {
                       var prop2 = new UnaryOperator(spreadContext.CombineWith(property.Context))
						{
							Operand = property,
							OperatorToken = JSToken.RestSpread,
							OperatorContext = spreadContext
        ...
        private ObjectLiteralProperty ParseObjectLiteralProperty(bool isBindingPattern)
        {
            ...
            else if (m_currentToken.IsEither(JSToken.Get, JSToken.Set))
            {
                ...
                var funcExpr = ParseFunction(isGet ? FunctionType.Getter : FunctionType.Setter, funcContext, false);
            ...
            else if (m_currentToken.Is(JSToken.Multiply) || nextToken == JSToken.LeftParenthesis)
            {
                ...
                value = ParseFunction(FunctionType.Method, m_currentToken.Clone(), false);
                if (value != null)
                {
        ...
        private AstNode ParseMemberExpression(AstNode expression, List<Context> newContexts)
        {
            for (; ; )
            {
                switch (m_currentToken.Token)
                {
                    ...
                    case JSToken.ElvisConditional:
                    case JSToken.AccessField:

                        if (m_currentToken.Token == JSToken.ElvisConditional)
                        {
                            if (expression is Lookup lookup1)
                            {
                                lookup1.IsElvisOperator = true;
                            }
                            else if (expression is Member member1)
                            {
                                member1.IsElvisOperator = true;
                            }
                            else if (expression is CallNode callNode1)
                            {
                                callNode1.IsElvisOperator = true;
                            }
                            else
                            {
                                ReportError(JSError.IllegalChar, expression.Context, true);
                            }
                        }
                        ...
                        string name = null;
                        // we want the name context to start with the dot or ?.
                        ...
                        if(m_currentToken.Is(JSToken.LeftBracket))
						{
                            ...
                            AstNode accessor2 = ParseExpression();
                            if (accessor2 != null)
                            {
                                args.Append(accessor2);
                            }
                        ...
                        else if (m_currentToken.IsNot(JSToken.Identifier))
                        {
        ...
        {
            // eg. jQuery has module.exports="...", but it is ES5 sintax and not module, so do not assume it is module even if we have ES6 as ParsedVersion
