...
namespace Microsoft.Ajax.Utilities
{
    ...
    public sealed class JSScanner
    {
        ...
        public Context ScanNextToken()
        {
            ...
            switch (ch)
            {
                ...
                case '?':
                    ...
                    if ('?' == ch) // ??
                    {
                        ++m_currentPosition; // consume current char
                       token = JSToken.LogicalOrElvis;
                    }
                    else if ('[' == ch) // ?[ // this is not suported in JS, ony in C#
                    {
                        token = JSToken.ElvisConditional;
                    }
                    else if('.' == ch) // ?.
					{
                        ++m_currentPosition; // consume current char
            ...
            try
            {
                ...
                while ((ch = GetChar(m_currentPosition++)) != delimiter)
                {
                    if (ch != '\\')
                    {
                        ...
                        {
                            // maybe: we want to flag this string as unterminated *and having issues*,
            ...
            try
            {
                ...
                while (ch != '\0' && ch != '`')
                {
                    ...
                    else if (IsLineTerminator(ch, 1))
                    {
                        ...
                        // gurufix: remove increment, it solves problem when string `...` has ending (`) on new line with no chars in front
                        //++m_currentPosition;
        ...
        private static OperatorPrecedence[] InitOperatorsPrec()
        {
            ...
            operatorsPrec[JSToken.LogicalOrElvis - JSToken.FirstBinaryOperator] = OperatorPrecedence.LogicalOr2;
