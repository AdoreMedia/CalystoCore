...
namespace Calysto.TypeLite {
	...
	public class TypeScriptFluent {
		...
		public TypeScriptFluent() {
			...
			_scriptGenerator = new TsGenerator(_modelBuilder);

			// gurufix: ignore namespaces
			this.IgnoreNamespace("System")
				.IgnoreNamespace("Microsoft")
				.IgnoreNamespace("System.ComponentModel.Annotations")
				.IgnoreNamespace("Calysto.Web.Script.Services.WebSockets");

			// gurufix: allow namespace
			this.AllowNamespace("Microsoft.AspNetCore.Html.HtmlString");
		}
		...
		public TypeScriptFluent For<T>(bool ignore = false)
		{
			return this.For(typeof(T), ignore);
		}

		public TypeScriptFluent ForResx<T>(bool ignore = false)
		{
			var model = _modelBuilder.AddResx<T>();
			if (model is TsClass)
			{
				((TsClass)model).IsIgnored = ignore;
				return this;
			}
			return this;
		}
		...
		/// <returns>Instance of the TypeScriptFluent that enables fluent configuration.</returns>
		public TypeScriptFluent For(Type type, bool ignore=false)
		{
		...
		/// <summary>
		/// Add all classes from assembly with T type.
		/// </summary>
		/// <typeparam name="T"></typeparam>
		/// <returns></returns>
		public TypeScriptFluent ForAssemblyWith<T>()
		{
			_modelBuilder.Add(typeof(T).Assembly);
			return this;
		}

		public TypeScriptFluent IgnoreAssembly(string assemblyName)
		{
			_modelBuilder.IgnoreAssembly(assemblyName);
			return this;
		}

		/// <summary>
		/// Has lower precedence than AllowNamespace.
		/// </summary>
		/// <param name="fullPath">Namespace and type name.</param>
		/// <returns></returns>
		public TypeScriptFluent IgnoreNamespace(string fullPath)
		{
			_modelBuilder.IgnoreNamespace(fullPath);
			return this;
		}

		/// <summary>
		/// Has higher precedence than IgnoreNamespace.
		/// </summary>
		/// <param name="fullPath">Namespace and type name.</param>
		/// <returns></returns>
		public TypeScriptFluent AllowNamespace(string fullPath)
		{
			_modelBuilder.AllowNamespace(fullPath);
			return this;
		}
		...
		public Dictionary<TsGeneratorOutput, string> GenerateAll()
		{
			var model = _modelBuilder.Build();
			return Enum.GetValues(typeof(TsGeneratorOutput)).Cast<TsGeneratorOutput>().ToDictionary(o => o, o => _scriptGenerator.Generate(model, o));
		}
