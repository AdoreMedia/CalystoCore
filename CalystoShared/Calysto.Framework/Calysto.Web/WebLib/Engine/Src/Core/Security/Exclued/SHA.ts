namespace Calysto.Security.Cryptography
{
	namespace shaEngine
	{
		...
		class Int_64
		{
			public highOrder: number;
			public lowOrder: number;
		...
		{
			var bin:number[] = [], codePnt: number, binArr: number[] = [], byteCnt = 0, i, j, offset;
		...
		{
			var bin:number[] = [], length:number = str.length, i, num, offset;
	 ...
	 */
		...
		{
			var bin:number[] = [], codePnt, i, offset:number;
		...
		{
			var retVal: number[] = [], byteCnt = 0, index, i, j, tmpInt, strPart, firstEqual, offset,
		...
		function getOutputOpts(outputOpts)
		{
			...
			try
			{
				if (outputOpts && outputOpts.hasOwnProperty("outputUpper"))
				{
					retVal["outputUpper"] = outputOpts["outputUpper"];
				}

				if (outputOpts && outputOpts.hasOwnProperty("b64Pad"))
				{
		 ...
		 */
		function rotr_64(x, n): Int_64
		{
			var retVal: Int_64, tmp = new Int_64(x.highOrder, x.lowOrder);
		...
		{
			var W:any[] = [], a, b, c, d, e, T, ch = ch_32, parity = parity_32,
		...
		function coreSHA2(message, messageLen, variant)
		{
			var a, b, c, d, e, f, g, h, T1, T2, H, numRounds, lengthPosition, i, t,
				...
				gamma0, gamma1, sigma0, sigma1, ch, maj, Int, W:any[] = [], int1, int2, offset,
		...
		export class jsSHA
		{
			private utfType: string;
			private strToHash = [0];
			private strBinLen = 0;

			constructor(srcString:string|number[], inputFormat:string, encoding:string)
			{
				var srcConvertRet: any;

				this.utfType = encoding || "UTF8";

				if (!(("UTF8" === this.utfType) || ("UTF16BE" === this.utfType) || ("UTF16LE" === this.utfType)))
				{
				...
				if ("HEX" === inputFormat)
				{
					...
					{
						throw Error("srcString of HEX type must be in byte increments");
					}
					srcConvertRet = hex2binb(srcString);
					this.strBinLen = srcConvertRet["binLen"];
					this.strToHash = srcConvertRet["value"];
				}
				else if (("TEXT" === inputFormat) || ("ASCII" === inputFormat))
				{
					srcConvertRet = str2binb(srcString, this.utfType);
					this.strBinLen = srcConvertRet["binLen"];
					this.strToHash = srcConvertRet["value"];
				}
				...
				{
					throw Error("inputFormat must be HEX, TEXT, ASCII, B64, or BYTES");
				}
			}
			 ...
			 *   specified
			 */
			public getHash(variant : string, format : string, numRounds?: number, outputFormatOpts?)
			{
				var formatFunc:any, message = this.strToHash.slice(),
					messageBinLen = this.strBinLen, i;
			 ...
			 *   specified
			 */
			public getHMAC(key, inputFormat, variant, outputFormat, outputFormatOpts)
			{
				var formatFunc, keyToUse, blockByteSize, blockBitSize, i,
					retVal, lastArrayIndex, keyBinLen, hashBitSize,
					keyWithIPad:any[] = [], keyWithOPad:any[] = [], keyConvertRet: any;
				...
				{
					keyConvertRet = str2binb(key, this.utfType);
					keyBinLen = keyConvertRet["binLen"];
					keyToUse = keyConvertRet["value"];
				}
				else if ("B64" === inputFormat)
				...
				if (blockByteSize < (keyBinLen / 8))
				{
					...
					{
						throw Error("Unexpected error in HMAC implementation");
					}
	...
	/// <summary>
	/// Compute hash of str string.<br/>Returns HEX string.<br/>
	/// Compatible with .NET System.Security.Cryptography.SHA1.Create().ComputeHash(Encoding.UTF8.GetBytes("some text is this?")).Select(o => o.ToString("x2")).ToStringJoined()
	/// </summary>
	/// <param name="str" type="String">(required) Input string.</param>
	/// <param name="outputType" type="String" optional="true">B64, HEX, or BYTES, default is HEX</param>

	function ComputeHashImpl(
		input: string | number[],
		hashType: "SHA-1" | "SHA-224" | "SHA-256" | "SHA-384" | "SHA-512" = "SHA-1",
		inputType: "HEX" | "ASCII" | "TEXT" | "BYTES" | "B64" = "TEXT",
		outputType: "B64" | "HEX" | "BYTES" = "HEX") : string
	{
		return new shaEngine.jsSHA(input, inputType, "UTF8").getHash(hashType, outputType || "HEX");
	}

	export namespace SHA1
	{
		export function ComputeHash(input: string | number[]) { return ComputeHashImpl(input, "SHA-1", "TEXT", "HEX"); }
	}

	export namespace SHA224
	{
		export function ComputeHash(input: string | number[]) { return ComputeHashImpl(input, "SHA-224", "TEXT", "HEX"); }
	}

	export namespace SHA256
	{
		export function ComputeHash(input: string | number[]) { return ComputeHashImpl(input, "SHA-256", "TEXT", "HEX"); }
	}

	export namespace SHA384
	{
		export function ComputeHash(input: string | number[]) { return ComputeHashImpl(input, "SHA-384", "TEXT", "HEX"); }
	}

	export namespace SHA512
	{
		export function ComputeHash(input: string | number[]) { return ComputeHashImpl(input, "SHA-512", "TEXT", "HEX"); }
	}
}
