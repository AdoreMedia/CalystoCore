...
{
	[TestClass()]
	public class SelectParallelExtensionsThreadsTests
	{
		...
		public void SelectParallelTest001()
		{
			...
			Task.Run(async () =>
			{
				var query1 = GetCollectionValues().Select((o,n)=>new {item=o, index = n }).SelectParallel2(async s =>
				{
					await Task.Delay(10);
					return new { s, res = "e1-" + s };

				}, 20, 0).SelectParallel2(async s=>new { s, res = "e2-" + s }, 20, 0).SelectParallel2(async s=>new { s, res = "e3-" + s }, 50, 50);
		...
		public void SelectParallelTest002()
		{
			...
			Task.Run(async () =>
			{
				var query1 = GetCollectionValues().Select((o, n) => new { item = o, index = n }).SelectParallel2(async s =>
				{
					await Task.Delay(10);
					return new { s, res = "e1-" + s };
				}, 20, 0).SelectParallel2(s => Task.Run(()=> new { s, res = "e2-" + s }), 20, 0).SelectParallel2(s => new { s, res = "e3-" + s }, 50, 50);
		...
		public void SelectParallelTest003()
		{
			...
			Task.Run(async () =>
			{
				var query1 = GetCollectionValues().SelectParallel2(async s =>
				{
					await Task.Delay(10);
					return s;
				}, 50);
