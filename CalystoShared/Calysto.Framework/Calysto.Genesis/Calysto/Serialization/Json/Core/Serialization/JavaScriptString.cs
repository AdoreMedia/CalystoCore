...
namespace Calysto.Serialization.Json.Core.Serialization
{
    internal class JavaScriptString
    {
        ...
        {
            return string.Concat(new object[] { message, " (", this._index, "): ", this._jsstr });
        }

        internal char? GetNextNonEmptyChar()
        {
            while (this._jsstr.Length > this._index)
            {
                char c = this._jsstr[this._index++];
        ...
        {
            if (this._jsstr.Length > this._index)
            {
                return new char?(this._jsstr[this._index++]);
            }
            return null;
        }
        ...
        {
            if (this._jsstr.Length >= (this._index + count))
            {
                string str = this._jsstr.Substring(this._index, count);
                this._index += count;
                return str;
            }
            return null;
        ...
        public override string ToString()
        {
            if (this._jsstr.Length > this._index)
            {
                return this._jsstr.Substring(this._index);
            }
            return string.Empty;
        }
    }
